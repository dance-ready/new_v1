...
(function(){
  ...
  /* ── HERO PARALLAX ──
     Translates hero images upward as the user scrolls so the full dancer
     silhouette (head→feet) is revealed instead of staying cropped. */
  (function(){
    ...
    function collect(){
      ...
      phs.forEach(function(ph){
        ...
        if(img) targets.push({ container: ph, img: img });
      });
      var heroStage = document.getElementById('hero-slides');
      if(heroStage){
        var heroSec = heroStage.closest('.hero') || heroStage.parentElement;
        targets.push({ container: heroSec, stage: heroStage });
      }
    }
    collect();
    ...
    function update(){
      ...
      targets.forEach(function(t){
        ...
        var h = rect.height || 1;
        // progress 0 → 1 as the container scrolls from fully-in-view to fully-out
        var p = Math.min(Math.max(-rect.top / h, 0), 1);
        var shift = -(p * MAX_SHIFT * 100); // percent of image height
        var transform = 'translate3d(0,' + shift.toFixed(2) + '%,0)';
        if(t.img){
          t.img.style.transform = transform;
        } else if(t.stage){
          var imgs = t.stage.querySelectorAll('.hslide img');
          for(var i=0;i<imgs.length;i++) imgs[i].style.transform = transform;
        }
      });
    }
    function onScroll(){
    ...
    // Re-collect hero-slides images once the slideshow IIFE has populated them
